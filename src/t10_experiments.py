...
MODEL_DIR_PATH = "../models/re-id/real50artificial50/"
...
full_model_name = f"{model_name}-real50artificial50"


for i in range(2, len(dates) + 1):
    gal_pbs = dataset.get_pbs_by_date(dates[i-2])
    query_pbs = dataset.get_pbs_by_date(dates[i-1])
    if i == len(dates):
        query_pbs = dataset.get_undamaged_pbs(query_pbs)
    test = ReidExperiment(exp_name="t10",
...
gal_pbs = dataset.get_pbs_by_date(dates[-1])
gal_pbs = dataset.get_undamaged_pbs(gal_pbs)
query_pbs = dataset.get_pbs_by_date(dates[-1])
damaged_pbs = dataset.get_damaged_pbs(query_pbs)

test_damaged = ReidExperiment(exp_name="t10",
                        model_name=full_model_name,
